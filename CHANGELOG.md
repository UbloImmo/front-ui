...
- `v1.2.2` Form component
  ...
  - Do not render any table field if no columns are visible

## 2.5.2 - 2026-09-04
### Changed
- Updated the package's peer dependencies
...
- `v0.1.1` Grid layout
  ...
  - Added support for CSS `minmax()` declarations when defining columns & rows
- `cssMinmax()`, `cssMinmaxWithData()`, `parseCssMinmax()` utility functions
- Added missing CSS unit related type predicates

### Changed
- Updated & fixed some typings
  - Converted some CSS unit related types to generics, so their value is not lost:
    - `CssVar`
    - `CssVarName`
    - `CssVarUsage`
  - Restricted `CssLengthUsage` type's union member `CssVarUsage` to `CssVarUsage<SpacingLabel>`
  - Added a generic overload to the `cssLengthUsage` utility function. This change helps TS narrow down the resulting value's type based on its input, therefore hopefully reducing type complexity in its usage down the line.

### Fixed
- `v1.2.1` Form component
  
  - Only hide non-selected table rows in read-mode when the `selectable` modifier is enabled
- The `isCssLengthUsage` predicate now properly accounts for `CssVarUsage<SpacingLabel>` candidates and rejects non-wrapped `SpacingLabel` candidates.
...
- Updated `@ubloimmo/front-tokens` to `^0.2.6`
...
## 2.2.1 - 2026-03-17
### Fixed
- `v0.0.2` ListTableHeaderSort component
  - Adjusted cell label style in case of no sort found.
  - Updated cell label test id to match component.
- `v0.1.1` ListTableHeaderFilter component
  - Updated cell label test id to match component.
...
- `0.1.0` List component
  - Refactored list filter options internal logic & data structure:
    ...
    - Improved large-volume management by reducing execution complexity.
  - Updated `FilterData`'s `optionSignatures` to be a Set instead of an array
...
## 2.0.4 - 2026-02-18
### Fixed
- `v0.1.2` Hypertext component
  - Ensure inner text's display property is properly applied.

## 2.0.3 - 2026-02-16
### Fixed
- Storybook online documentation configuration by downgrading storybook to `v10.1.11`.
- `v0.1.1` Hypertext component
  - Fixed inner text's overflow behavior.
